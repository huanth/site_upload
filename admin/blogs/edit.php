...
<?php if (isset($current_login_user)) :
    if ($current_login_user['role'] == 1 || $current_login_user['role'] == 0) :
        ...
        if (isset($_POST['update_blog'])) {
            ...
            $sql = "UPDATE blogs SET title = ?, content = ?, hot = ?, pin = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param('ssiii', $title, $content, $hot, $pin, $id);
            if ($stmt->execute()) {
                ...
                echo '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 mb-4 rounded relative" role="alert">
                        ...
                        <span class="block sm:inline">Cập nhật blog thất bại.</span>
                    </div>';
            }
        }
?>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-2xl mx-auto">
                <form action="" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="title" name="title" required class="w-full p-2 border border-gray-300 rounded-lg focus:ring focus:ring-green-300" placeholder="Title" value="<?= $blog['title']; ?>">
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="hot" id="hot" class="w-full p-2 border border-gray-300 rounded-lg focus:ring focus:ring-green-300">
                            <option value="0" <?= $blog['hot'] == 0 ? 'selected' : ''; ?>>Không</option>
                            <option value="1" <?= $blog['hot'] == 1 ? 'selected' : ''; ?>>Có</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="pin" id="pin" class="w-full p-2 border border-gray-300 rounded-lg focus:ring focus:ring-green-300">
                            <option value="0" <?= $blog['pin'] == 0 ? 'selected' : ''; ?>>Không</option>
                            <option value="1" <?= $blog['pin'] == 1 ? 'selected' : ''; ?>>Có</option>
                        </select>
                    </div>

                    <div class="mb-4">
                        <button type="submit" name="update_blog" class="bg-primary text-white px-4 py-2 rounded-lg">Cập nhật</button>
                    </div>
                </form>
            </div>
